<template>
  <div>
    <div
      :style="cssVariables"
      v-if="kind == LoaderAnimation.SimpleSpinner"
      class="simple-spinner"
    ></div>
    <div
      :style="cssVariables"
      v-else-if="kind == LoaderAnimation.DoubleRectangleSpinner"
      class="rotating-rectangle"
    ></div>
    <div
      :style="cssVariables"
      v-else-if="kind == LoaderAnimation.RotatingBalls"
...
import {LoaderAnimation} from "@enums";
import {ref,computed} from 'vue';

const _props = withDefaults(
  defineProps<{
    kind?: LoaderAnimation;
    primaryColor?: string;
  ...
  {
    kind: LoaderAnimation.SimpleSpinner,
    primaryColor: "#3979a4",
    height: 40,
    width: 40,
    size: 8,
...
const adjustedSize = computed<number>(() => {
  ...
  switch (_props.kind) {
    case LoaderAnimation.DoubleRectangleSpinner:
      return 2;
    case LoaderAnimation.RotatingBalls:
      return 18;
    default:
      return _props.size;
  }
